...
> **Draft extraction from PR #20; not release-ready.** This preserves the July
> implementation for focused review. Local baseline: 26 tests pass. Before
> merge, add explicit partial-scan/error reporting, enforce `.blastcontainignore`,
> verify malformed registry responses and identifier escaping, prevent local
> draft filename collisions/overwrites, and distinguish advisory report signatures
> from attestation. The CLI currently does not establish complete inventory or
> validate the security of discovered assets.
...
```bash
pip install -e ./core -e ./discovery
