...
> **Draft server extraction from PR #20; not production-ready.** Local baseline:
> 101 server tests pass against current Core/Guard. Authentication currently
> becomes optional when no API token is configured; actor names come from the
> request; invalid or unsigned finding packets can affect lifecycle state.
> Before merge, establish fail-closed authentication, caller/agent/environment
> authorization, trusted evidence requirements for state changes, and installed
> wheel/API-schema validation. The console and historical designs are separate
> draft PRs; links to them become available when those changes land.
...
```bash
uvicorn --app-dir server server:app --host 127.0.0.1 --port 8080
```
