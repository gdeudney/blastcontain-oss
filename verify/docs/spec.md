...
| **What it checks** | Reads `--context-file`. **Presidio** (if active): `AnalyzerEngine` with `en_core_web_lg` model for `CREDIT_CARD`, `US_SSN`, `EMAIL_ADDRESS`, `PHONE_NUMBER`, `IBAN_CODE`, `PERSON`, `LOCATION`. **Fallback regex** (always runs): email, phone, SSN, credit card patterns. **AGT** `PromptDefenseEvaluator` (if active): runs after Presidio. |
